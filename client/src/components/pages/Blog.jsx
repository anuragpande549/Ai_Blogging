...
import { getBlogData } from '../../context/fetchData'
...
const fetchData = await getBlogData("/blogs/get-blog", id);
    setData(fetchData.data)
    setName(fetchData.data.userDetails)
  }
  ...
  return data ? (
    <div className='relative'>
      ...
      <div className='text-center mt-20 text-gray-600'>
        <p className='text-primary py-4 font-medium'>Published On {Moment(data.blogDetails.createdAt).format('MMMMM Do YYYY')}</p>
        <h1 className='text-2xl sm:text-5xl font-semibold max-w-2xl mx-auto text-gray-800'>{data.blogDetails.title}</h1>
        <h2 className='my-5 max-w-lg truncate mx-auto'>{data.blogDetails.subTitle}</h2>
        <p className='inline-block py-1 px-4 rounded-full mb-6 border text-sm border-primary/35 font-medium text-primary'>{name.name}</p>
      </div>

      <div className='mx-5 max-w-5xl md:mx-auto my-10 mt-6'>
        <img src={data.blogDetails.image} alt="" className='rounded-3xl mb-5'/> 
        <div dangerouslySetInnerHTML={{__html:data.blogDetails.description}} className='rich-text'></div>
      </div>
      ...
      <div className='my-24 max-w-3xl mx-auto'>
        ...
        <p className='font-semibold my-4'>Share this post in social media </p>
        <div className='flex'>
